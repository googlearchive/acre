 ...
 *  @constructor
 */
mjt.freebase.SchemaCache = function (service) {
    this.service = service;
    this.cache = {};
};

mjt.freebase.SchemaCache.prototype.get = function(tid) {
...
mjt.freebase.SchemaCache.prototype.lookup = function(tid) {
    return this.cache[tid];
};


mjt.freebase.SchemaCache.prototype.make_schema_query = function (tid) {
    return {
        id: tid,
        name: null,
        type: '/type/type',
 ...
 */
mjt.freebase.Schema = mjt.define_task(null, [{name: 'typeid'}]);


mjt.freebase.Schema.prototype.toString = function() {
    return '[' + this._task_id + ' ' + this.typeid + ']';
};


mjt.freebase.Schema.prototype.init = function() {
    this.props = {};
    this.props_by_id = {};

    var tq = mjt.freebase.schema_cache.make_schema_query(this.typeid);
    this._mqlread = mjt.freebase.MqlRead(tq);
 ...
 *
 */
mjt.freebase.Schema.prototype.request = function() {
 ...
 *
 */
mjt.freebase.Schema.prototype.handle_response = function(r) {
    this.name = r.name;
 ...
 */
mjt.freebase.SchemaCache.prototype.getcore = function() {
    if (this.corefetch)
        return this.corefetch;

    var preload_types = ['/type/object', '/type/value'];
    var multi = this.corefetch = mjt.freebase.MqlReadMultiple();

    // pull some other language spec stuff - directives, ...
    // multi.mqlread(tidk, mjt.freebase.MqlRead(...);

    for (var i = 0; i < preload_types.length; i++) {
        var tid = preload_types[i];
        var tidk = tid;
        multi.mqlread(tidk, mjt.freebase.MqlRead(mjt.freebase.schema_cache.make_schema_query(tid)));
    }

    return multi.enqueue();
};

mjt.freebase.schema_cache = new mjt.freebase.SchemaCache(mjt.freebase);

...
class test_class(TestController):
    ...
    def test_require_freebase_id(self):
        data ={ "url": "/freebase/apps/qatests/api/mql_query_test.mql" };
        self.set_acre_script("require", self.encode_data(data));
        expected_query =  { "id":"/user/acretestbot", "type":[] }; 
        expected_result = {"url":"/freebase/apps/qatests/api/mql_query_test.sjs","name":"mql_query_test","query":{"id":"/user/acretestbot","type":[]}}
        ...
        try: 
            ...
            self.assert_(result['name'] == "mql_query_test.mql", "Got an unexpected name.  Got: %s Expected: %s " % ( result['name'], "mql_query_test.mql") );

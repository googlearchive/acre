...
function _table(label1,value1,label2,value2) {
    var t = '\n<table><tbody>';
    if (label1) { t += '<tr><td class="subhead">'+ label1 +'</td>'+ _displayStringForValue(value1) +'</tr>\n'; }
    if (label2) { t += '<tr><td class="subhead">'+ label2 +'</td>'+ _displayStringForValue(value2) +'</tr>\n'; }
    t += '</tbody></table>\n';
    return t;
}
...
function _displayStringForValue(aVar) {
    var result = '<td class="value">' + aVar + '</td>';
    if (!(aVar === null || aVar === JSUNIT_UNDEFINED_VALUE)) {
        result += ' <td>(' + _trueTypeOf(aVar) + ')</td>';
    }
    return result;
}
...
//TODO: delete this - or merge functionality with html version
function run_tests(tests) { 
    for (var funcName in tests) {
        try { 
            tests[funcName]();
            console.info(funcName+' - OK');
        } catch (e) {
            var comment = '';
            if (e.comment) { comment = ' - "'+e.comment + '"'; }
            console.error(funcName+' - FAIL: '+e.jsUnitMessage + comment );
        }
    }
}

function run_nose_tests(tests) { 
    for (var funcName in tests) {
        try { 
            tests[funcName]();
            acre.write(funcName+' - OK');
            console.info(funcName+' - OK');
        } catch (e) {
            var comment = '';
            if (e.comment) { comment = ' - "'+e.comment + '"'; }
            acre.write(funcName+' - FAIL: '+e.message + comment );
            console.error(funcName+' - FAIL: '+e.jsUnitMessage + comment );
        }
    }
}

function run_html_tests(tests) {
    acre.write('<table class="tablesorter" id="results"><thead><tr><th>Test</th><th>Pass</th><th>Message</th><th>Comment</th></tr><tbody>'); 
    for (var funcName in tests) {
        acre.write('<tr>');
        try { 
            tests[funcName]();
            acre.write('<td>'+funcName+'</td><td style="color:green">OK</td>');
        } catch (e) {
            console.log('function '+funcName+ '\tException: '+JSON.stringify(e));
            // DEBUG: for (var p in e) { console.log('p='+p+' val='+e[p]); }
            // e.message, e.lineNumber etc are direct from rhino (e.jsUnitMessage, e.comment are from jsUnit) //TODO: merge?
            acre.write('<td>'+funcName+'</td><td style="color:red">Fail</td><td>'+ (e.jsUnitMessage || e.message) +'</td><td class="comment">'+ (e.comment||'') + '</td>');
        }
        acre.write('</tr>');
    }
    acre.write('</tbody></table>');
}


function run_html_test_page(tests) {
    acre.response.status = 200;
    acre.response.headers['content-type']= 'text/html; charset="UTF-8"'
acre.write('<html> \
<head> \
  <title>Acre Tests</title> \
  <script type="text/javascript" src="http://code.jquery.com/jquery.js"></script> \
  <script type="text/javascript" src="http://mqlx.com/~willmoffat/libs/jQuery/jquery.tablesorter.js"></script> \
  <link rel="stylesheet" type="text/css" href="http://mqlx.com/~willmoffat/libs/jQuery/tablesorter/style.css"/> \
  <link rel="stylesheet" type="text/css" href="/api/trans/raw/user/willmoffat/acre-tests/css"/> \
</head> \
<body onload="$(\'.tablesorter\').tablesorter();"> \
<h1>Test results</h1>');
    run_html_tests(tests);
}

// TEST:
// run_html_test_page(example_tests);

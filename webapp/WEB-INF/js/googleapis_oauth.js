...
(function() {
    ...
    **/
    function getProvider(provider) {
        ...
        if (!provider.access_token_URL) {
            throw new oauthError("The OAuth provider '" + provider.name + "' is missing the necessary 'access_token_URL' property");
        }
        if (!provider.user_authorization_URL) {
            throw new oauthError("The OAuth provider '" + provider.name + "' is missing the necessary 'user_authorization_URL' property");
        }
        if (!provider.domain) {
            throw new oauthError("The OAuth provider '" + provider.name + "' is missing the necessary 'domain' property");
        }
        if (provider.oauth_version == 2) {
            ...
            if (!provider.request_token_URL) {
                throw new oauthError("The OAuth provider '" + provider.name + "' is missing the necessary 'request_token_URL' property");
            }
        }
        return provider;
    };
    ...
    **/
    function getConsumer(provider) {
        ...
        try {
            ...
            throw new oauthError("Error while retrieving the application key for provider '" + provider.name + "': " + e);
        }
        if (!key) {
            throw new oauthError("Could not find the OAuth key for provider '" + provider.name + "' in the keystore. Have you added the key to the keystore yet?");
        }
        return key;
    }
    ...
    **/
    function getCredentials(url, token, consumer) {
        ...
        if (!credentials) {
            throw new oauthError("Could not find OAuth credentials to sign a request to '" + url +
                            "'. Have you invoked 'acre.oauth.get_authorization' with the right provider?");
        }
